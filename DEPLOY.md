...
onnxruntime run the embedding graph on four threads instead of one. `embed` is
the dominant stage in the query: 16.3 ms of a 21.5 ms browser query, and 51 ms
of it single-threaded. Without the headers it falls back silently -
**nothing errors, nothing logs, the app is just three times slower than it was
on localhost.**

The headers are necessary but not sufficient: onnxruntime-web still runs on one
thread unless it is given a number, which `web/src/retrieval/encoder.worker.ts`
does.
...
| `SARVAM_API_KEY` | real speech input (streaming), and spoken answers |
...
|---|---|---|
| App shell (JS + CSS + HTML, gzipped) | 56 KB | your origin |
| onnxruntime WASM | 5.1 MB gz | your origin |
| Model weights (e5-small int8) | ~135 MB | Hugging Face CDN |
| Index vectors | ~66 MB | your origin, 75.9 MB raw, ~1.1x compressible |
| Index passage text | ~14 MB | your origin, 77.8 MB raw, brotli 5.7x |

Roughly **200 MB cold, and 2.6 s warm** (13.1 s cold on localhost, where the
bytes cost nothing). Index blobs go into IndexedDB keyed by the manifest's build
timestamp; model weights go into the browser's Cache API. The second visit
fetches neither.
