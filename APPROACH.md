...
technical name for the technique. A flame you ask questions of, out loud, in any
of fourteen Indian languages — that answers from its own library or from
documents you hand it, in under 200 milliseconds.
...
| | |
|---|---|
| Typical question (P50) | **7.7 milliseconds** |
| **The single worst question (P100)** | **9.5 milliseconds** |

The target was 200. The worst case came in at a twentieth of that.

Two changes since our first version halved the worst case. We moved the
language model that reads the question onto a **separate thread**, so it stops
competing with everything else the page is doing. And we made the system
**practise before it starts** — the very first question any program answers is
always slow, because the machine hasn't yet learned to run that code
efficiently, and if you don't practise first, that one slow question becomes
your worst-case score.

Searching all six versions of the documents takes about **2 milliseconds**. Most
of the remaining time is spent understanding the question, not finding the answer.
...
The system held its deadline perfectly all the way down to 12 milliseconds —
meaning it would still meet the real 200ms target on a machine roughly
seventeen times slower than ours. Below about 10 milliseconds, the budget is
smaller than the single unavoidable step of reading the question, which no
amount of cleverness recovers. We report that floor rather than trimming it off
the bottom of the table.

**This test paid for itself immediately by finding two real bugs** — both in
code that simply never runs on a fast machine, so no amount of ordinary testing
...
Speaking back is split between two services, and the split is the interesting
part. ElevenLabs has the better voice but, of the fourteen languages here, it
only speaks Hindi and Tamil. Sarvam speaks the other eight Indic ones. So each
answer goes to whichever service can actually say it. Using just one would have
meant choosing between a good voice and being understood in Kannada.

If neither service is configured, the browser's own built-in voice fills in —
and the interface says so, in plain words, rather than letting you assume you
were hearing the real thing.

---

## Asking in fourteen languages
...
**It works, and it fades gracefully.** Hindi finds the right passage 59% of the
time — that's the ceiling, since Hindi is what the library is written in.
English is nearly level with it. Bengali, Nepali and Malayalam land around 40%.
Assamese and Sanskrit sit at the bottom near 25%, both being languages the
