/**
 * The words in a string that carry its subject.
 *
 * Gates 2 and 3 compare a query or an answer against a passage by counting
 * shared words, and a short spoken question is mostly function words where
 * prose is not:
 *
 *     "what is the name"   vs   "name: srinidhi bhat, age: 45, sex: M"
 *
 * Four tokens, one shared: overlap 0.25, under gate 2's rescue floor, while the
 * bare query "srinidhi" scores 1.0 on the same passage. The dilution is all
 * `what`, `is`, `the`, and it gets worse the more politely you ask.
 *
 * A script with no list here is not filtered, same rule as the input gate. A
 * partial stoplist strips the wrong words and shifts that language against a
 * threshold fitted on English and Hindi.
 */

/**
 * English function words. Short on purpose: this is not an IR stoplist and is
 * not trying to improve ranking. "name", "number" and "date" stay in, because
 * in a question about a document they are often the whole subject.
 */
 ...
 *
 * The corpus is Hindi and the threshold was fitted on it, so this is the one
 * non-Latin script where leaving function words in moves a calibrated number.
 * Marathi rides along because the script does not separate them.
 */
 ...
 * combining marks, not letters, so a `\p{L}`-only class cuts every word at its
 * first matra: `भारत` becomes `र` + `त` and `निगम क्या है` reduces to `गम`.
 * Both sides of an overlap comparison get mangled the same way, so the score
 * stays plausible while measuring the wrong thing. ZWNJ and ZWJ stay for the
 * same reason: Devanagari uses them for conjuncts.
 */
 ...
 *
 * Falls back to unfiltered tokens when filtering leaves nothing: a query that
 * is all function words names no subject, and an empty set scores every overlap
 * at 0, which looks the same as an unrelated passage.
 */
 ...
 *
 * Decides whether comparing two strings word by word means anything, so the
 * label is coarse ("latin", "deva", "beng"). The question is not which
 * language, only whether the two can share a vocabulary.
 */
 ...
 * and a grounding check that disagrees rejects a correct answer for using the
 * reader's numerals.
 */
